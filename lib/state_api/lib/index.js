...
class DataAPI{
    ...
    generateSubscribeFunc(field){
        if(this.subscribers[field]){
            console.log("FAILED TO GENERATE NEW SUBSRIBE FUNCTION (FIELD TO SUBSCRIBE WASNT UNIQUE)");
            throw err;
        }
    ...
    generateUnsubscribeFunc(field){
        if(!this.subscribers[field]){
            console.log("NO SUBSCRIBTION TO CREATE AN UNSUBSCRIBE FUNCTION FOR ");
            throw err;
        }
    ...
    generateNotifyFunc(field){
        if(!this.subscribers[field]){
            console.log("NO SUBSCRIBTION TO CREATE A NOTIFY FUNCTION FOR ");
            throw err;
        }
        return () =>{
            const KEY = 0, VALUE = 1;
...
//TODO might need to bind sheet
 //subscribeToSearchTerm = this.generateSubscribeFunc('searchTerm' );
    //unsubscribeToSearchTerm = this.generateUnsubscribeFunc('searchTerm');
  //  notifyUpdateToSearchTerm = this.generateNotifyFunc('searchTerm');
